...
```{r message=FALSE}
source("../MOSresults/downloadTCGA/functions-to-process-TCGA-data.R")
...
load("../MOSresults/downloadTCGA/rawData.RData")
```
...
```{r}
fup<-suppressWarnings(read_excel("../MOSresults/downloadTCGA/1-s2.0-S0092867418302290-mmc1.xlsx", sheet=1, range = "B1:AH11161"))
fup <- fup[fup$type=="OV",]
fup <- list(os=data.frame(row.names=fup$bcr_patient_barcode,
                          status=fup$OS,
                          days=fup$`OS.time`, stringsAsFactors = F),
            pfs=data.frame(row.names=fup$bcr_patient_barcode,
                           status=fup$PFI,
...
```{r eval=FALSE}
save(classes, fup, metClustValues, ov.cnv, ov.mutations, expAvg, file="/MOSresults/downloadTCGA/TCGA-OV-pre-processed.RData")
```
