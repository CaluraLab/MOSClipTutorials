...
If you was enough attentive, you noticed that we didn't perform the patient selection for the expression matrix. That is because we are going to perfom some extra steps after dropping the patients of this matrix. We need to filter the genes to avoid data sparsity, keeping only those genes with at least 100 counts in at least one patient. 
...
At this point, we need a pathway knowledge-base. We will use the Reactome pathway graphs available at *graphite* R package. Reactome is natively distributed in Uniprot, our analysis will be in Entrez gene ID, thus we need to convert the pathway identifiers.

Since we are downloading and converting all the Reactome pathways, this step may take a while. To avoid the boredom of waiting for this step again, we will save the object for future analysis.
```{r}
...
```
```{r echo=FALSE}
save(multiOmics, file = paste0(dirname, "multiOmics.RData"), compress = TRUE)
```
...
For both types of analysis, we perform another *a priori* filter, using the genes that are present at least in the expression data, but this is not mandatory.
...
## Module Analysis
Here we are going to perform the multi-omics survival analysis on pathway **modules**. 
...
if (file.exists(paste0(dirname, "momSurvivalList.RData"))) {
    ...
    save(momSurvivalList, file = paste0(dirname, "momSurvivalList.RData"), compress = TRUE)
}
```
...
```{r warning=FALSE}
if (file.exists(paste0(dirname, "mopSurvivalList.RData"))) {
    load(paste0(dirname, "mopSurvivalList.RData"))
} else {
    mopSurvivalList <- lapply(reactHuge, function(g) {
        print(g@title)  # uncomment it so you can see which pathway is being analyzed
        #for each pathway contained in reactSmall, create the MOM object
    ...
    save(mopSurvivalList, file = paste0(dirname, "mopSurvivalList.RData"), compress = TRUE)
}
```
...
MOSClip have a function that can plot a heatmap of the report of the results. The heatmap is sorted by the p-value of the Cox proportional hazard model, using all the omics as covariates. The leftmost column is the p-value of the model, and the other columns are the p-values for each covariate. The color gradient also corresponds to the p-values. In this way, the plot can help to understand the involvement of different omics in the survival. 

### Report as heatmap
For the module analysis, the function *plotReport()* will plot all the modules of a chosen pathway. We will select the correspondent `MultiOmicsModules` object from the list of modules results.
```{r}
plotReport(momSurvivalList[["Activation of Matrix Metalloproteinases"]], 
           MOcolors = c(exp = "red", met = "green", cnv = "yellow"))
```

As you can see in this heatmap, the module 5 of the pathway "Activation of Matrix Metalloproteinases" is significant. Furthermore, we can infer the omics that drives this survival behavior from the pvalues of the model covariates: expression (expPC1 and PC2), and methylation (met2k2).

For the pathway analysis, we can plot the first n pathways in a list of `MultiOmicsPathway` objects:
```{r}
plotReport(mopSurvivalList, top = 20, 
...
plotKaplanMeier(momSurvivalList[["Activation of Matrix Metalloproteinases"]], 5,
                formula = "Surv(days, status) ~ expPC1 + met2k",
                paletteNames = "Paired", inYears = TRUE)

plotKaplanMeier(mopSurvivalList[["Activation of ATR in response to replication stress"]], 
...
plotHeatmap(momSurvivalList[["Activation of Matrix Metalloproteinases"]], 5,
               sortBy = c("expPC1", "met2k", "status", "days"),
               additionalAnnotations = survAnnot,
               additionalPaletteNames = list(status = "teal", days = "violet"),
               withSampleNames = F)

plotHeatmap(mopSurvivalList[["Activation of ATR in response to replication stress"]],
...
Here we can see that we have 29 modules with their expression and methylation significantly associated with survival. In the pathways graph, we have instead 14 significant pathways with the combination of expression and CNV.
...
correspondence <- lapply(names(modulesIntersections), function(omicClass) {
    data.frame(path = modulesIntersections[[omicClass]], 
               father = modules2fathers[[omicClass]], 
               stringsAsFactors = F)
})

plotFrequencies(MOMfreqDf, width = 12, maxSize = 12)
```
...
plotFrequencies(pathwayFreqDf, width = 12)
```

Note that here is not necessary to perform the step to trip the module number from the pathway names, since we are not dealing with modules.


### Module in graph plot
The function *plotModuleInGraph()* is specific for the module-type analysis, and it allows the visualization of the position of a chosen module inside its pathway. The function takes as input a `MultiOmicsModules` object, a `PathwayList` object from the *graphite* R package, which in our case is the variable *reactSmall*, and the module number we want to visualize:
```{r message=FALSE}
plotModuleInGraph(momSurvivalList[["Activation of Matrix Metalloproteinases"]],
                  pathList = reactSmall, moduleNumber = 5, 
                  paletteNames = c(exp = "red", met = "green", cnv = "yellow"))
```


# Resampling strategy
So far, we have identified some modules or pathways that are significant. However, MOSClip gives the possibility to prioritize the most important and stable results with the resampling strategy. In this way, modules or pathways with a p-value <= 0.05 are resampled n times (customized) and later on are selected those successful above a certain threshold. For each iteration, the analysis is run on a new cohort of patients generated by randomly removing a specific number of patients (e.g., 1% of total patients).
In practice, this is done as below:
```{r warning=FALSE}
useThesePathwaysM <- unique(na.omit(moduleSummary$pathway[moduleSummary$pvalue <= 0.05]))
...
```
In our case we used 100 as number of permutations, removing 3 patients per permutation. Note that depending on the number of permutation set, this step could take a while to run. 

For the pathway-type analysis is pretty much the same, only that the resampling function is different:
...
runSupertest(moduleSummary, pvalueThr = 0.05, zscoreThr = 0.05,
             resamplingThr = 80, excludeColumns = c("pathway", "module",
                                                   "resamplingCount"))
```
...
plotReport(mopSurvivalList[names(mopSurvivalList) %in% resamplingTrue], 
           top = 20,
           MOcolors = c(exp = "red", met = "green", mut = "blue", cnv = "yellow"))
```
